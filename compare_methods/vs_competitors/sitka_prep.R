...
# opt$outfile = "../data/cadd456b3a9334a5/sitka_input.csv"
...
get_single_feature_bins <- function(dat, mat_delta, bin_lenght = 1e6) {
  ...
  min(delta_bin_dat$dist)
  stopifnot(min(delta_bin_dat$dist, na.rm = T) == bin_lenght)
  
  # Keep bins that only cover 500K bases
  single_feature_bins <- delta_bin_dat$delta_bin_names[delta_bin_dat$dist == bin_lenght]
...
read_input = function(input) {
  ...
  bins = df$bin %>% unique()
  cell_ids = unique(df$sample_id)
  
  c_id = cell_ids[1]
  
  M = lapply(cell_ids, function(c_id) {
    df %>% 
      dplyr::filter(sample_id == c_id) %>% 
      dplyr::arrange(bin) %>% 
      dplyr::pull(CN)  
  }) %>% do.call(cbind, .)
  colnames(M) = cell_ids
...
M = read_input(input = opt$input)
pip_CNV_2_corrupt(input_data = M, opt$outfile)
